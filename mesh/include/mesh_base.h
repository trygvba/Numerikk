...
class MeshBase {
  public:
    ...
    MeshBase(struct triangulateio* in, bool opt_edge=false,
              double area_constraint = -1.);	// constructor taking in triangulateio struct
    ~MeshBase();
  ...
  private:
    ...
    char* get_triswitches();  // Get char* pointer to be used by Triangle.
